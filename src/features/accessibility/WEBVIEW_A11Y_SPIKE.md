...
| `epub.js` iframe/content focus (TalkBack cannot reach content) | Critical/Blocking, but now attributed to platform, not app | **Unchanged — still Critical/Blocking as a user-facing defect, but see the 2026-09-10 correction above §12.6.** Confirmed via 3 independent touch points on content with correct bounds (§12.3). Bounds were necessary but not sufficient. **Caveat added 2026-09-10**: the same tap-exact-bounds method is also silent on plain non-clickable *native* content with nothing to do with WebView/epub.js — so this rating reflects "the tooling used so far can't rule out F4," not a confirmed WebView-specific defect independent of that tooling gap. **2026-09-11, decisive and independent of that confound**: an in-process instrumentation test calling `AccessibilityNodeInfo.performAction(ACTION_ACCESSIBILITY_FOCUS)` directly on the WebView content node — no synthetic touch/gesture at all, so the tap-exact-bounds confound doesn't apply — got `true` back from the action call, but a fresh re-query of the live tree 500ms later showed `isAccessibilityFocused() == false`. The action is accepted, not rejected, but never durably lands. **2026-09-14, H1 finally run properly (direct-action method, not tap) — identical symptom reproduces in stock Chrome, on a page with no iframe at all AND on a same-origin iframe, with zero involvement from this app, epub.js, or `react-native-webview`.** Rules out iframe-specificity and app-specificity both. What remains is very likely a Chromium/Android-WebView accessibility-bridge limitation on this OS/browser build (H4) — not something fixable in this repo. Full account: `02_a11y_root_cause_RESULT.md`'s 2026-09-14 section. Only open question left: whether a real TalkBack gesture (as opposed to the direct API call used throughout) fares differently — needs real hardware or a userdebug/eng AVD (confirmed unavailable in this environment: `tts_spike` is a `user`-type build, `adb root` explicitly refused). |
