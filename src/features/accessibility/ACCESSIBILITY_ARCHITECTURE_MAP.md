...
| `tts.highlightMode` | Low | **Engine half done 2026-09-02; WebView rendering handed to Ahana** | Native `tts-progress` is subscribed, normalized per-platform (`ttsProgress.ts`), and gated on `highlightMode === 'word'` in `useTtsSession.ts`, forwarding word offsets through a new `setSpokenWordRange` on `ReaderTextProvider`/`readerBridge.ts` (all tested). `webview/src/bridge.ts`'s `CommandArgs` and `epub.entry.ts`/`pdf.entry.ts` don't implement it yet — that's a real `npm run typecheck` failure today, by design. Concrete handoff spec: `API_CONTRACT_NOTES.md` §6. Still not rendered as a control since there's nothing on-screen to show yet. **Caught and fixed the same day:** the first version called `setSpokenWordRange` unconditionally on every TTS start/stop regardless of mode, which — since the WebView has no handler — broke default `'sentence'` mode for everyone via `ReaderScreen`'s interrupting error banner; both call sites are now gated on `highlightMode === 'word'` |
