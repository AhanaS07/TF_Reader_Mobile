...
| `flow` | EPUB: sets `READER_FLOW`. `isPaginated()` already gates the line grid and column breaks, so this is the value that constant becomes. **Changing flow live needs a re-layout** — see §5. PDF (added for continuous scroll, Ahana): `flow: 'scrolled-doc'` switches `pdf.entry.ts` from its single-canvas renderer into a virtualised, scrollable multi-page one (`enterScrollMode`/`leaveScrollMode`) — the one PDF field beyond `bg`/`zoom` this shell now applies. |
