...
/**
 * TEMPORARILY points at `assets/20mb_EPUB.epub` (a real, structurally valid ~20MB EPUB) instead
 * of the generated `assets/reader/sample-plaintext.epub` placeholder, for a like-for-like memory
 * remeasurement against the real ~15MB `test-15mb.pdf` fixture below — a 3.6KB placeholder vs a
 * 15MB PDF is not a format comparison. Same rules as the PDF swap: LOCAL, not a design change
 * Reader has signed off on; `assets/20mb_EPUB.epub` is gitignored and must STAY that way. Revert
 * to `../../../assets/reader/sample-plaintext.epub` before this file is deleted or before
 * committing changes that touch this file, whichever comes first.
 */
...
const SAMPLE_EPUB_MODULE = require('../../../assets/20mb_EPUB.epub') as number;
...
/**
 * TEMP, with the rest of this file.
 *
 * TEMPORARILY points at `assets/test-15mb.pdf` (a real, structurally valid ~15MB PDF) instead of
 * the generated `assets/reader/sample-plaintext.pdf` placeholder, at Abhinav's explicit request,
 * for on-device PDF-path testing with something pdf.js can meaningfully page through. This is a
 * LOCAL SWAP, not a design change Reader (Ahana) has signed off on — `assets/test-15mb.pdf` is
 * untracked and must STAY untracked (T4_Readme.md: never commit real content). Revert this
 * `require()` back to `../../../assets/reader/sample-plaintext.pdf` before this file is deleted
 * or before committing changes that touch this file, whichever comes first — the generated
 * placeholder remains the CI-checked, byte-reproducible fixture `reader:build-sample-pdf`
 * produces; this override does not replace it.
 *
 ...
 */
const SAMPLE_PDF_MODULE = require('../../../assets/test-15mb.pdf') as number;
...
// 4: SAMPLE_PDF_MODULE's source bytes changed (temporarily points at assets/test-15mb.pdf, not
//    the generated placeholder — see that const's own comment). Without this bump, a simulator
//    that already seeded the old tiny placeholder under DEV_SAMPLE_PDF_BOOK_ID would
//    short-circuit on isAvailableOffline() and keep serving the OLD content forever, silently.
// 5: SAMPLE_EPUB_MODULE's source bytes changed the same way (temporarily points at
//    assets/20mb_EPUB.epub, not the generated 3.6KB placeholder) — same reasoning, this time for
//    DEV_SAMPLE_EPUB_BOOK_ID. Caught by hand while remeasuring: a stale seed here would silently
//    make an "EPUB vs PDF" memory comparison actually a "3.6KB vs 15MB" one.
