...
const api: TFReaderApi<'openPdf'> = {
   ...
   * `ReaderTextProvider` for a PDF book. Segmentation is what TTS needs and segmentation is EPUB-only
   * — `epubTtsResolver.ts` walks a rendered chapter's text nodes, and pdf.js has no equivalent this
   * could resolve an offset against. Defined because `TFReaderApi<'openPdf'>` requires every SHARED
   * command in both shells. */
  setSpokenWordRange: () => {},

  /** Documented no-op for the same reason as `setSpokenRange`/`setSpokenWordRange` above: TTS never
   * mounts for a PDF book, so nothing ever tells this shell it started or stopped speaking, and there
   * is no gesture-lock to apply. Defined only because `TFReaderApi<'openPdf'>` requires every SHARED
   * command in both shells. */
  setTtsSpeaking: () => {},
