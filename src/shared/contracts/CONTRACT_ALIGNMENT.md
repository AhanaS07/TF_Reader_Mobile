...
**Owner: Ahana (lead, `src/shared/contracts/`). Status as of 2026-08-26.**
...
| `B6` | 🟠 Change feed unimplemented — the designed revocation channel | 🟢 **Reader's half landed too, 2026-09-03** — `sync/offlineLock.ts` diffs `downloads.isValid` on every pull and emits `content.lock`/`content.unlock`; Encryption's subscriber (destroy the BEK on `reason: 'revoked'`) is wired and operational; Reader now subscribes for the WHILE-OPEN case via `src/features/reader/useContentLock.ts` (mid-open/reading/audio all covered — see CLAUDE.md's "Offline-lock gating hook"). `content.unlock` is deliberately NOT consumed by Reader — a reversed revocation while a book is open stays locked until the screen remounts, an accepted fail-closed choice, not a gap. Contract's 5 open questions resolved: Q1 ✅ (endpoint: server-written `isValid`), Q2 ✅ (destroy immediately), Q4 ✅ (memory-only removed — not a lock concern, known at download), Q5 ✅ (`reason: 'unknown'` means no confirmation, UI shows uncertainty). Q3 still open: synced-column constraint vs. design. | Abhinav/Karthik/Ahana | `download/`, `sync/`, `reader/` |
...
| `B8` | 🟠 `POST /device/register-key` doesn't exist; flambeau rejects the concept | ✅ **closed 2026-09-07** — `deviceKeyRegistration.ts`(+test) and `shared/contracts/device-key.ts` deleted, `DownloadError.REGISTRATION_FAILED` and the barrel export removed. Was DRAFT, never in `__typecheck__.ts`'s canary, so no freeze break — barrel edit still pending Ahana's sign-off | Abhinav + Ahana (barrel) | `download/`'s §5 |
